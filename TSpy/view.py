...
def split_series(X, groundtruth, min_zero_length=10):
    splits = []
    groundtruth_splits = []
    ...
    for i in range(len(X)):
        if np.all(X[i] == 0):
            ...
            if in_zero_run and zero_run_length >= min_zero_length:
                splits.append(X[start_idx:i-zero_run_length])
                groundtruth_splits.append(groundtruth[start_idx:i-zero_run_length])
                start_idx = i
            in_zero_run = False
    
    # Append the final segment
    splits.append(X[start_idx:])
    groundtruth_splits.append(groundtruth[start_idx:])
    
    return splits, groundtruth_splits
...
def plot_mts(X, groundtruth=None, prediction=None, figsize=(18,2), show=False):
    ...
    if groundtruth is None and prediction is None:
        ...
        if groundtruth is not None:
            ...
            plt.plot(X)
            ax1.xaxis.set_major_locator(plt.MaxNLocator(nbins=30, prune='both'))
            ax1.tick_params(axis='x', which='major', labelsize=8, top=False, bottom=True, left=False, right=False, labelleft=False, labelbottom=True)
...
def visualize_trace(X, groundtruth, min_zero_length=10):
    X_splits, groundtruth_splits = split_series(X, groundtruth, min_zero_length)
    for i, (X_split, groundtruth_split) in enumerate(zip(X_splits, groundtruth_splits)):
        plt.style.use('classic')
        plot_mts(X_split, groundtruth=groundtruth_split, show=False)
        plt.savefig(f'2fft_conv128m100plot_{i}.png')
        plt.close()
