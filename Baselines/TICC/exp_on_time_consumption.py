...
def effect_of_length(time_series_id):
    data = np.loadtxt('/home/lh/E2USD/data/synthetic/test'+str(time_series_id)+'.csv', delimiter=',')
    data = np.concatenate([data[:,:4] for x in range(30)])
...
if __name__ == '__main__':
    ...
    for i in range(1,11):
        ...
        np.savetxt('./effect_of_length/time'+str(i)+'.txt', t_list)
    ...
    time_list = []
    for i in range(1,11):
        time_list.append(np.loadtxt('./effect_of_length/time'+str(i)+'.txt'))
