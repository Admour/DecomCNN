...
def exp_on_trace(win_size, num_seg, offset):
    import pca
# seq_len = args.slid
    data_path = "C:\\Ipdu_stastics\\X2E_npy\\X2E_297_01096_20240318_130813_20240326_171322_no_someip\\pdu_data_segment_0.npy"
    ...
    data = df_pivot.to_numpy().astype(int)
    # data = np.load(data_path)
    # time_path_data = "C:\\Ipdu_stastics\\X2E_npy\\pdu_time.npy"
    # pdu_time = np.load(time_path_data, allow_pickle=True)
    # pdu_time = [time_str/10 for time_str in pdu_time]
    # data = pca.pca_process(data, 18)
    prediction = run_clasp(data, window_size=win_size, num_cps=num_seg, n_states=10, offset=offset)
    prediction = np.array(prediction, dtype=int)
    plt.style.use('classic')
    np.save("prediction.npy", prediction.state_seq)
    plot_mts(data, prediction)
    plt.savefig('no_someip2.png')
    visualize_trace(data, prediction, min_zero_length=10)
...
if __name__ == '__main__':
    ...
    try:
        exp_on_trace(200, 80, 0.001) 
    except Exception as e:
        print("An exception occurred: ", str(e))
        traceback.print_exc()
    # exp_on_trace(500, 20, 0.02)
