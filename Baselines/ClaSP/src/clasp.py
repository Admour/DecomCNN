...
def sliding_mean_std(TS, m):
    ...
    movmean = segSum / m
    val = np.maximum(segSumSq / m - (segSum / m) ** 2, 0)
    movstd = np.sqrt(np.maximum(segSumSq / m - (segSum / m) ** 2, 0))
...
def _calc_knn_labels(knn_mask, split_idx, window_size):
    ...
    if split_idx - window_size < 0:
        exclusion_zone = np.arange(max(0, split_idx - window_size), split_idx)
        y_pred[exclusion_zone] = np.ones(split_idx, dtype=np.int64)
...
def calc_clasp(time_series, window_size, k_neighbours=5, score=_roc_auc_score, interpolate=True, offset=.05):
